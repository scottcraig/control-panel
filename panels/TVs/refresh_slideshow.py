...
def refresh_slideshow(username=None):
    ...
    for tv in range(1, 5):
        ...
        if username in dir_contents:
            utils.print_success(
                "Found art for {} on TV# {}".format(username, tv))
...
def generate_new_movie_file(ssh_connection, username, tv, silent=False):
    ...
    # don't care about error if it doesn't exists
    ssh_connection.send_cmd('rm {}'.format(output), print_stdout=False)
    ...
    success = ssh_connection.send_cmd(
        'cp {} {}/tv{}/{}.a.mp4'.format(output, TV_ROOT, tv, username))
    ...
    if success == '':
        ...
        if tv != 4:
            success = ssh_connection.send_cmd(
                'cp {} {}/tv4/{}.a.mp4'.format(output, TV_ROOT, username))
            if success == '':
                if not silent:
