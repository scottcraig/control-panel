...
def view_or_remove_media(username=None, tv=None):
    ...
    while True:
        ...
        media_command = "cd {}/tv{}/ && find {}.z.*".format(
            TV_ROOT, tv, username)
        media_files = ssh_connection.send_cmd(
            media_command, print_stdout=False).split()
        ...
        delete_file = utils.confirm(
            "Would you like to delete {}? Hopefully it popped up for you".format(
                art_file),
        ...
        if not keep_going:
            ...
            refresh = utils.confirm(
                "Would you like to refresh the user's video slideshow?")
            if refresh:
